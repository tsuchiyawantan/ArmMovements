...
public:
	...
	void drawCurvedArmMove(cv::Mat &srcImg){
		int x0 = 0;
		int y0 = 50;
		int x1 = 10;
		int y1 = 40;
		...
		for (t; t < tf; t += 0.2){
			...
			xt += 300;
			yt += 300;
			srcImg.at<cv::Vec3b>(yt, xt)[0] = 255; // b; //ê¬
			srcImg.at<cv::Vec3b>(yt, xt)[1] = 255; // g; //óŒ
			srcImg.at<cv::Vec3b>(yt, xt)[2] = 0; // r; //ê‘
	...
	void drawLine(cv::Mat &srcImg, vector<pair<int, int>> &yx, int hue){
		...
		for (int i = 0; i < yx.size(); i++){
			...
			srcImg.at<cv::Vec3b>(y, x)[0] = 255; // b; //ê¬
			srcImg.at<cv::Vec3b>(y, x)[1] = 255; // g; //óŒ
			srcImg.at<cv::Vec3b>(y, x)[2] = 0; // r; //ê‘
			...
			for (double t = 0; t <= 1.0; t += 0.009){
				...
				srcImg.at<cv::Vec3b>(y, x)[0] = 255; // b; //ê¬
				srcImg.at<cv::Vec3b>(y, x)[1] = 255; // g; //óŒ
				srcImg.at<cv::Vec3b>(y, x)[2] = 255; // r; //ê‘
